...
model.compile(loss = 'categorical_crossentropy', optimizer='adam',metrics = ['accuracy']) #RMSprop(learning_rate=0.001)

# 创建一个只保存最好模型的检查点
checkpoint = ModelCheckpoint('best_2-model.h5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')

# model = Sequential()
# model.add(Embedding(max_fatures, embed_dim,input_length = X_train.shape[1])) #use word2Vec
# model.add(SpatialDropout1D(0.7)) # 增加Dropout比例
# model.add(LSTM(lstm_out, dropout=0.2, recurrent_dropout=0.2, return_sequences=True)) # 添加return_sequences=True以便在下一层使用序列输出
...
if accuracy_test > 0.58:
    os.rename('best_2-model.h5', f'LSTM-model-addLayer-{accuracy_test:.3f}.h5')
    # model.save(f'LSTM-model-addLayer-{accuracy_test:.3f}.h5')
