...
def augment_text(text):
    ...
    augmented_words = []
    for word in words:
        if random.random() < 0.1:  # 10% 的概率替换单词
            synsets = wordnet.synsets(word)
            if synsets:
                synonyms = synsets[0].lemmas()
                if synonyms:
                    augmented_words.append(synonyms[0].name())
                else:
...
df_train['text'] = df_train['text'].apply(augment_text)  # 对训练数据进行增强
...
# 创建一个只保存最好模型的检查点
checkpoint = ModelCheckpoint('best3_model.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='min')

# 添加早停
early_stop = EarlyStopping(monitor='val_loss', patience=5)
model.fit(X_train, pd.get_dummies(df_train['subreddit']).values, epochs = 14, batch_size=batch_size, verbose = 2, callbacks=[checkpoint, early_stop], validation_split=0.1) # 添加validation_split参数 early_stop,
